...
function AllExp({ distance }: any) {
  ...
  const data: any = ExpeditionData.find((obj) => obj.route === distance);
  ...
  return (
    <div className="w-11/12 mx-auto flex gap-4 pt-[2rem] py-[3rem] relative">
      {/* tab  */}
      <div className="w-[15%]">
        <div className="w-full h-auto sticky top-[5rem] left-0   rounded-lg  flex flex-col   justify-start  items-start gap-4">
          ...
          >
            <h1
              className={`relative font-semibold  text-sm text-wrap  w-full  py-3 group-hover:scale-105   duration-200  flex justify-start px-2 items-center    ${"text-zinc-800"}`}
            >
              All expedition
            </h1>
            {/* <Icon icon="ri:arrow-drop-right-line" className="" /> */}
          </Link>
          {ExpeditionData.map((item: any, index: number) => (
            <Link
            ...
            >
              <h1
                className={`relative font-semibold  text-sm text-wrap  w-full  py-3 group-hover:scale-105   duration-200  flex justify-start px-2 items-center    ${
                  item.route === distance ? "text-yellow-500" : "text-zinc-800"
                }`}
              >
                {item.route}
              </h1>
