...
const job = scheduleJob('1 2 * * *', async function () {
  ...
  const advancedBookings = await BookingModel.find({$and:[{
    paymentOption: "deposit-payment", startDate: { $gte: now, $lte: reminderDate }
...
const job2 = scheduleJob('1 2 * * *', async function () {
  ...
  const overdueBookings = await BookingModel.find({$and:[{
    remainingAmount: {$gte:0}, startDate: {  $lte: now }
