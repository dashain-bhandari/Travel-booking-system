...
import { createUser, deleteUser, findAllUser, findAllUserExceptAdmin, findAndUpdateUser, findManyUser, findUser, validatePassword } from "../service/user.service";
...
`;
    const info = await sendEmail({ to: req.body.email, subject: "Email Verification", message: message });
...
export async function createRoleSpecificUserHandler(req: Request<{}, {}, CreateUserInput["body"]>, res: Response, next: NextFunction) {
  try {
    const existingUserWithEmail = await findUser({ email: req.body.email });
    if (existingUserWithEmail) {
      return res.status(409).json({
    ...
    const hashedPassword = await generateHashedPassword(req.body.password);
    const createdUser = await createUser({ ...req.body, password: hashedPassword, isVerified: true, verifyToken: "" });
...
export async function loginUserHandler(req: Request<{}, {}, LoginUserInput["body"]>, res: Response, next: NextFunction) {
  try {
    ...
    // );
    // Generate a token with the user payload and secret key
    const accessToken = jwt.sign({ user }, `${process.env.AUTH_SECRET_KEY}`, { expiresIn: "45d" });
...
export async function updateRoleSpecificUserHandler(req: Request<UpdateUserInput["params"]>, res: Response, next: NextFunction) {
  try {
    const userId = req.params.userId;
    const update = req.body;
    ...
    const notification = await createNotification({
      title: `${user?.firstName ? user?.firstName : ""} ${user?.middleName ? user?.middleName : ""} ${user?.lastName ? user?.lastName : ""} of role ${user?.role} updated availability.`,
    });
...
export async function getAllUserExceptAdminHandler(req: Request, res: Response, next: NextFunction) {
  try {
...
export async function getRoleSpecificUserHandler(req: Request, res: Response, next: NextFunction) {
  try {
    const filter = req.query;
    const users = await findAllUser({ role: { $nin: ["user", "admin", "super-admin"] } });
    return res.json({
      status: "success",
      msg: "Get all user success",
      data: users,
...
const sendEmail = async ({ to, subject, message }: any) => {
  try {
    const info = await transporter.sendMail({
      from: '"Contour Expedition" <loki@webxnep.com>', // sender address
      to,
      subject,
...
export async function getUserAvailabilityHandler(req: Request<UpdateUserInput["params"]>, res: Response, next: NextFunction) {
  try {
...
export async function updateUserPasswordByAdminHandler(req: Request<UpdateUserInput["params"]>, res: Response, next: NextFunction) {
  try {
    const userId = req.params.userId;
...
export async function updatePasswordHandler(req: any, res: Response, next: NextFunction) {
  try {
    ...
    const hashedPassword = await generateHashedPassword(req.body.password);
    const updatedUser = await findAndUpdateUser({ email }, { password: hashedPassword }, { new: true });
