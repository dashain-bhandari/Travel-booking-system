...
export default function NewNav({ navData, other }: Props) {
  ...
  const NavLinks = [
    ...
    { id: 6, name: "Company", href: "/about_us" },
  ...
  const handleOpenSmallNavLink = (index: number, link: string) => {
    ...
    switch (link) {
      ...
      case "Other Activities":
        return showOpenClickedLinkNav(OtherActivitiesData);
      // case "Training":
      //   // Render training related content
    ...
    <>
      ...
      >
        <div className="w-11/12 4xl:w-10/12 flex justify-between items-center mx-auto h-full">
                ...
                <div className="w-11/12 4xl:w-10/12  mx-auto grid grid-cols-6">
                  {currentOpen && (
                    ...
                    >
                        ...
                        {currentOpen?.map((item: any, index: number) => {
                          ...
                          return (
                            <Link
                              ...
                              className={`text-left text-normal-paragraph-lg lg:text-normal-paragraph-lg font-primary capitalize  cursor-pointer  ${
                      ...
                      >
                        <CustomButton text="View all" arrow="false" />
                      </Link>
                    </motion.div>
                  )}
                        ...
                        .map((item: any, index: number) => {
                          ...
                          return (
                            <>
                              ...
                              >
                                <div className="absolute z-20 w-11/12 text-start text-normal-paragraph-lg lg:text-normal-paragraph-lg font-primary  pointer-events-none font-semibold  bottom-[5%] left-[5%] mx-auto  group-hover:opacity-[1] text-zinc-100  duration-200">
                                  {item?.name}
                                </div>
                                <Link
                                  href={`/trip/${item?.slug}`}
          ...
          {/* LINKS  */}
          <div className="gap-2 xl:gap-[2rem] hidden lg:flex  top-[4rem]  whitespace-nowrap  justify-center items-center">
                ...
                <span
                  ...
                  className={`nav-links ${
                    currentRoute === link.href
                      ? "text-yellow-500"
                      : "text-zinc-300"
                  }   font-semibold items-center text-nav-heading-md lg:text-nav-heading-lg font-primary hover:text-yellow-500 cursor-pointer flex gap-1`}
                >
                  {link.name}
                  {link.name === "Home" ||
                  link.name === "Company" ||
                    ...
                    >
                      {/* {CompanyLinks.map((link, index) => (
                        <Link
                          key={index}
                          href={link.href}
                          className="text-left hover:text-yellow-500 text-zinc-100 text-normal-paragraph-lg lg:text-normal-paragraph-lg font-primary cursor-pointer"
                        >
                          {link.name}
                        </Link>
                      ))} */}
            ...
            {navData?.slice(1, 2).map((item: any, index: number) => {
              console.log("col", item);
              return (
                <Link
                  href={`/expedition/${item.slug}?cat=${item.categories[0].slug}`}
                  key={index}
                  className="relative"
                >
                  <span
                    onMouseEnter={() => {
                      handleHoverLink(item.name), setCurrentCol(item.slug);
      ...
      <div className="fixed sideNav pt-[5rem] h-screen overflow-y-scroll top-0 lg:hidden left-0 w-full   translate-x-[100%] bg-[#1E1E1E] z-[50]">
        <div className="w-11/12   pr-3 overflow-y-scroll mx-auto text-zinc-200 h-full  flex flex-col  justify-center gap-5">
          ...
          <Link href="/customize_trip">
            {/* <button
              type="button"
              className="text-white w-8/12 mx-auto mb-5  hover:gap-4 duration-200 relative z-10 flex gap-2 items-center justify-center  bg-gradient-to-r from-yellow-500 via-yellow-600 to-yellow-700 hover:bg-gradient-to-br  focus:outline-none focus:ring-yellow-300 dark:focus:ring-yellow-800 font-medium rounded-md text-[12px] md:text-sm md:px-3 px-1 py-2 text-center "
            >
              Customize your trip <Icon icon="mynaui:arrow-long-right" />
            </button> */}
          ...
          {navData?.map((item: any, index: number) => (
            ...
            >
              <div className="flex w-full justify-between">
                <Link
                  onClick={handleOpenNav}
                  href={`/expedition/${item?.slug}?cat=${
                    item.categories?.length > 0 && item?.categories[0]?.slug
                  }`}
                  className={`${
                        ...
                        currentOpen?.map((item: any, i: number) => (
                          <div
                            ...
                            onClick={() => {
                              handleOpenNav();
                              router.push(
                                `/expedition/${currentCol}?cat=${item?.slug}`
                              );
                            }}
                          >
                            {item.name}
